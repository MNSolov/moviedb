...
export default class FilmCard extends PureComponent {
  constructor(props) {
    ...
    this.textOverviewClamp = (textOverview, overviewFontSize, lineHeightOverview, heightOverview, widthOverview) => {
      ...
      if (textOverview) {
        ...
        while (j < wordsOverview.length && i < countStringOverview) {
          ...
          if (temp * overviewFontSize * 0.8 < widthOverview) {
            result[i] += ` ${wordsOverview[j]}`
            j += 1
          } else {
            i += 1
    ...
    this.addOverview = () => {
      const cardHeight = this.refCard.current.getBoundingClientRect().height
      const headerHeight = this.refHeader.current.getBoundingClientRect().height
      const imageHeight = this.refImage.current.getBoundingClientRect().height
      let overviewHeight = 0
      const overviewWidth = this.refOverview.current.getBoundingClientRect().width

      if (cardHeight > imageHeight)
        overviewHeight = imageHeight > headerHeight ? cardHeight - imageHeight - 20 : cardHeight - headerHeight - 20
  ...
  render() {
    ...
    return (
      <div className="film-card" ref={this.refCard}>
        <img className="film-card__image" src={imgPath} alt="Poster" ref={this.refImage} />
